...
def compute_mse(mdl, X, y, print_first_five_results = False, lstm = False, if_tf = False, if_tc = False):
    ...
    if lstm:
        ...
        pred = np.array(pred).reshape(len(y), -1)
        #print(pred.shape, y.shape)
...
def contract_all_edges(tt):
    ...
    for i in range(1, len(tt)):
        #print(output.shape, tt[i].shape)
        output = np.tensordot(output, tt[i].tensor, (-1, 0))
    return output
...
def ALS(X, Y, rank, X_vali = None, Y_vali = None, n_epochs = 50):
    ...
    #tt = contract_all_edges(tt)
    tt = convert_to_array(tt)
    #error = evaluate(X, Y, tt)
    #print(error)
    return tt
