    ...
    for k in range(len(methods)):
        ...
        if method == 'TIHT+SGD':
            ...
            for i in temp_index_list:
                ...
                X_temp = np.swapaxes(X[i], 1, 2)

                Y_temp = Y[i].reshape(-1, 1)
                # X_temp = X_temp[:100]
                # Y_temp = Y_temp[:100]
                #print(X_temp.shape)
                H_temp = learning.ALS(X_temp, Y_temp, rank=num_states, X_vali=None, Y_vali=None, n_epochs=ALS_epochs)
                #print(H_temp)
                H_vec.append(H_temp)
            ...
            #     for j in range(len(H_vec[i])):
            #         print(H_vec[i][j].shape)
            #     print(H_vec[i][-1])
            learned_model = TT_learning.TT_spectral_learning(H_vec[1], H_vec[2], H_vec[0])
            print(learned_model.alpha)
            T = toc(t)
            pred = []
            for o in Xtest:
            ...
            pred_k, ytest_k = pred_k_more(learned_model, X_test_temp, y_test_temp, pred, 2)
            ...
            pred_k, ytest_k = pred_k_more(learned_model, X_test_temp, y_test_temp, pred, 5)
